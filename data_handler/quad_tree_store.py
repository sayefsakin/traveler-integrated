...
class QuadTreeStore(data_queries.DataQueriesInterface):
    pass
...
class QDTStore:
    ...
    def insertIntoQDTree(self, start_loc_index, end_loc_index, st_time, en_time):
        ...
        if start_loc == end_loc:
            ...
            if st is None or en is None:
                if st is not None:
                    ...
                    return QDTNode(start_loc_index, end_loc_index, st_time, en_time)
                if en is not None:
            ...
            if st + 1 == en:  # this belongs to a single interval
                if self.parsed_data.sortedEventsByLocation[start_loc][st][1]['Event'] == 'ENTER' \
                    ...
                    st_time = max(self.parsed_data.sortedEventsByLocation[start_loc][st][1]['Timestamp'], st_time)
                    en_time = min(self.parsed_data.sortedEventsByLocation[start_loc][en][1]['Timestamp'], en_time)
                    left = QDTNode(start_loc_index, end_loc_index, st_time, en_time - 1)
                    right = QDTNode(start_loc_index, end_loc_index, en_time, last_time)
                    return QDTNode(start_loc_index, end_loc_index, st_time, last_time, left, right)
                    ...
                    st_time = max(self.parsed_data.sortedEventsByLocation[start_loc][st][1]['Timestamp'], st_time)
                    en_time = min(self.parsed_data.sortedEventsByLocation[start_loc][en][1]['Timestamp'], en_time)
                    left = QDTNode(start_loc_index, end_loc_index, f_time, st_time)
                    right = QDTNode(start_loc_index, end_loc_index, st_time + 1, en_time)
                    return QDTNode(start_loc_index, end_loc_index, f_time, en_time, left, right)
                elif self.parsed_data.sortedEventsByLocation[start_loc][st][1]['Event'] == 'LEAVE' \
                        and self.parsed_data.sortedEventsByLocation[start_loc][en][1]['Event'] == 'ENTER':
                    last_time = en_time
                    f_time = st_time
                    ...
                    left = QDTNode(start_loc_index, end_loc_index, f_time, st_time)
                    right = QDTNode(start_loc_index, end_loc_index, en_time, last_time)
                    return QDTNode(start_loc_index, end_loc_index, f_time, last_time, left, right)
                else:
                    st_time = max(self.parsed_data.sortedEventsByLocation[start_loc][st][1]['Timestamp'], st_time)
                    en_time = min(self.parsed_data.sortedEventsByLocation[start_loc][en][1]['Timestamp'], en_time)
                    return QDTNode(start_loc_index, end_loc_index, st_time, en_time)
        ...
        up_left = self.insertIntoQDTree(start_loc_index, mid_v, st_time, mid_h)
        up_right = self.insertIntoQDTree(start_loc_index, mid_v, mid_h+1, en_time)

        down_left = None
        down_right = None
        if mid_v+1 <= end_loc_index:
            down_left = self.insertIntoQDTree(mid_v+1, end_loc_index, st_time, mid_h)
            down_right = self.insertIntoQDTree(mid_v+1, end_loc_index, mid_h+1, en_time)

        return QDTNode(start_loc_index, end_loc_index, st_time, en_time, up_left, up_right, down_left, down_right)
