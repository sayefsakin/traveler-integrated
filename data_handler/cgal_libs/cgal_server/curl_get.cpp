...
public:
    ...
    UrlParser(string url, Document params) {
        ...
        cout << "url parser initiated with values" << endl;
    }
    UrlParser() {
        baseUrl = "http://localhost:8000";
        urlParameters.SetNull();
        cout << "url parser initiated" << endl;
    }
...
private:
    string makeUrlString(CURL *curl) {
        string urlString;
        if(baseUrl.empty()) { cout << "please set URL first" << endl; return urlString;}
...
int main(void)
{
  ...
  cout << d["load_components_action"]["name"].GetString() << endl;
  return 0;
}*/