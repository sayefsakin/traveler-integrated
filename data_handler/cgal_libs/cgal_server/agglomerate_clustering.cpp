...
vector<double> EventAgglomerateClustering::binnedRangeQuery(int64_t time_begin, int64_t time_end, uint64_t bins) {
    ...
    //   cout << fixed << setprecision(0) << "i: " << i << " " << data[i*2] << " " << data[(i*2)+1] << " " << labels[i] << endl;
    // }
...
//     fstream input_file(input_file_path + "14.location");
...
//     vector<double> results = agglomerate_clustering.binnedRangeQuery(1074655386, 1246477086, 20);
//     PRINTLOG("Results: ");
